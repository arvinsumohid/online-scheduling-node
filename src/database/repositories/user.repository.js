...
const find = async (filter, projection = {}, options = {}, skip = 0, limit = 10) => {
  const userRes = await User.find(filter, projection, options).skip(skip).limit(limit);

  return userRes;
};

const findOne = async (filter, projection = {}, options = {}) => {
  const userRes = await User.findOne(filter, projection, options);
...
const updateOne = async (filter, update, options = {}) => {
  const userRes = await User.updateOne(filter, update, options);
